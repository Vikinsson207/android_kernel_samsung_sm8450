...
static int cam_otp_hi847_read_memory(struct cam_eeprom_ctrl_t *e_ctrl,
...
static int cam_otp_hi1337_read_memory(struct cam_eeprom_ctrl_t *e_ctrl,
                               struct cam_eeprom_memory_block_t *block);
...
#endif
	{
		...
		CAM_DBG(CAM_EEPROM, "[CAL] index:%d valid(%d, %d)", e_ctrl->soc_info.index, isValidIdx(ADDR_M_AF, &ConfAddr), isValidIdx(ADDR_S0_AF, &ConfAddr));
		{
			...
			if (e_ctrl->soc_info.index == SEC_WIDE_SENSOR) {
				...
				cam_eeprom_module_info_set_afcal(ADDR_S0_AF, rear_idx, sizeof(rear_idx)/sizeof(rear_idx[0]),
...
#endif
#else       //  #if defined(CONFIG_SEC_B0Q_PROJECT)
			AfIdx_t rear_idx[] = {
				{AF_CAL_NEAR_IDX, AF_CAL_NEAR_OFFSET_FROM_AF},
				{AF_CAL_FAR_IDX, AF_CAL_FAR_OFFSET_FROM_AF},
				{AF_CAL_M1_IDX, AF_CAL_M1_OFFSET_FROM_AF},
			...
			cam_eeprom_module_info_set_afcal(ADDR_S0_AF, rear_idx, sizeof(rear_idx)/sizeof(rear_idx[0]),
				e_ctrl->cal_data.mapdata, rear3_af_cal_str, sizeof(rear3_af_cal_str));
#endif      //  #if defined(CONFIG_SEC_B0Q_PROJECT)
		}

#elif defined(CONFIG_SAMSUNG_REAR_DUAL)     //  #if defined(CONFIG_SAMSUNG_REAR_TRIPLE)
		/* AF Cal. data read */
#if defined(CONFIG_SEC_GTS8P_PROJECT) || defined(CONFIG_SEC_GTS8U_PROJECT)
		{
			AfIdx_t rear_idx[] = {
				{AF_CAL_NEAR_IDX, AF_CAL_NEAR_OFFSET_FROM_AF},
				{AF_CAL_FAR_IDX, AF_CAL_FAR_OFFSET_FROM_AF},
			};
			CAM_INFO(CAM_EEPROM, "AF CAL - TAB");
	...
	struct cam_eeprom_ctrl_t *e_ctrl)
{
	...
	if (e_ctrl->soc_info.index == 1 || e_ctrl->soc_info.index == 8 ) {
		rc = cam_eeprom_dump(e_ctrl->soc_info.index, e_ctrl->cal_data.mapdata, 0x0000, 0x7DF);
	}
...
eeropm_crc_check :
		...
		e_ctrl->cam_eeprom_state = CAM_EEPROM_CONFIG;
		{
...
#elif defined(CONFIG_SEC_GTS8P_PROJECT)
			...
			if (1 < e_ctrl->cal_data.num_map) {
				...
				if (rc < 0) {
					CAM_ERR(CAM_EEPROM, "cam_eeprom_get_phone_ver failed");
					goto power_down;
				}
			}
		}
